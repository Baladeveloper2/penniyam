...
export async function generateStaticParams() {
  ...
  const staticParams = courses.map((course) => ({
    id: course.id,
  }));

  // Add static- prefixed IDs for hardcoded courses if necessary
  const hardcodedParams = STATIC_COURSES.map(c => ({
    id: c.id
  }));

  return [...staticParams, ...hardcodedParams];
}
      ...
      <div className="container" style={{ padding: 'clamp(2rem, 5vw, 5rem) 0', maxWidth: '1400px' }}>
        <div className="course-layout-grid">
          ...
          <div style={{ marginTop: '2rem' }}>
            <a
              href={`https://wa.me/919787831261?text=Hi, I'm interested in the ${course.title} course. Can you provide more details?`}
              target="_blank"
